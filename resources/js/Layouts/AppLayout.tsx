...
export default function GuestLayout({ header, children }: Props) {
    const page = usePage();
    const { auth, siteVisits, profileIncomplete, unreadNotifications, adminPhone, footerCategories } = page.props as any;
    ...
    // Worker profile pages have their own floating button that contacts the
    // worker directly — the site-wide contact button steps aside there.
    const showFloatingWhatsApp = whatsAppLink && page.component !== 'Workers/Show';
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col overflow-x-hidden">
            ...
            {/* Floating WhatsApp contact (site-wide) */}
            {showFloatingWhatsApp && (
                <a
                    href={`${whatsAppLink}?text=${encodeURIComponent(t('home.whatsappMessage'))}`}
                    target="_blank"
